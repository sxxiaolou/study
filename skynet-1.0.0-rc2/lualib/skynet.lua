	...
	if co == nil then
		co = coroutine.create(function(...)
			...
			while true do
				...
				-- if(skynet.self() == 13) then
				-- 	print("[:'log']--['file':skynet.lua]--['fun':co_create]--['self:']",skynet.self())
				-- else
				-- end
				f = coroutine_yield "EXIT"
				-- if(skynet.self() == 13) then
				-- 	print("[:'log']--['file':skynet.lua]--['fun':co_create yield EXIT]--['self:']",skynet.self())
				-- else
				-- end
				f(coroutine_yield())
			end
		...
		-- if(skynet.self() == 13) then
		-- 	print("[:'log']--['file':skynet.lua]--['fun':co_create coroutine_resume]--['self:']",skynet.self())
		-- else
		-- end
...
local function dispatch_wakeup()
	...
	if co then
		...
		if session then
			...
			-- if(skynet.self() == 13) then
			-- 	print("[:'log']--['file':skynet.lua]--['fun':dispatch_wakeup]--['self:']",skynet.self())
			-- else
			-- end
...
function suspend(co, result, command, param, size)
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':suspend]--['self:']",skynet.self(),result,command,param,size)
	-- else
	-- end
	if not result then
	...
	elseif command == "RETURN" then
		...
		-- if(skynet.self() == 13) then
		-- 	print("[:'log']--['file':skynet.lua]--['fun':RETURN coroutine_resume]--['self:']",skynet.self())
		-- else
		-- end
	...
	elseif command == "RESPONSE" then
		...
		-- if(skynet.self() == 13) then
		-- 	print("[:'log']--['file':skynet.lua]--['fun':RESPONSE coroutine_resume]--['self:']",skynet.self())
		-- else
		-- end
...
function skynet.timeout(ti, func)
	...
	assert(session)
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':timeout co_create]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.sleep(ti)
	...
	assert(session)
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':sleep]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.wait(co)
	...
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':wait]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.exit()
	...
	-- quit service
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':exit]--['self:']",skynet.self())
	-- else
	-- end
...
local function yield_call(service, session)
	...
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':yield_call]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.ret(msg, sz)
	msg = msg or ""
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':ret]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.response(pack)
	...
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':response]--['self:']",skynet.self())
	-- else
	-- end
...
function skynet.fork(func,...)
	...
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':fork co_create]--['self:']",skynet.self())
	-- else
	-- end
...
local function raw_dispatch_message(prototype, msg, sz, session, source)
	...
	-- if(skynet.self() == 13) then
	-- 	print("[:'log']--['file':skynet.lua]--['fun':raw_dispatch_message]--['self:']",skynet.self(),prototype,msg,sz,session,source)
	-- else
	-- end
	if prototype == 1 then
		local co = session_id_coroutine[session]
		...
		elseif co == nil then
			...
			session_id_coroutine[session] = nil
			-- if(skynet.self() == 13) then
			-- 	print("[:'log']--['file':skynet.lua]--['fun':raw_dispatch_message coroutine_resume]--['self:']",skynet.self())
			-- else
			-- end
		...
		if f then
			...
			-- if(skynet.self() == 13) then
			-- 	print("[:'log']--['file':skynet.lua]--['fun':raw_dispatch_message co_create]--['self:']",skynet.self())
			-- else
			-- end
			...
			-- if(skynet.self() == 13) then
			-- 	print("[:'log']--['file':skynet.lua]--['fun':f raw_dispatch_message coroutine_resume]--['self:']",skynet.self())
			-- else
			-- end
...
function skynet.dispatch_message(...)
	...
	while true do
		...
		fork_queue[key] = nil
		-- if(skynet.self() == 13) then
		-- 	print("[:'log']--['file':skynet.lua]--['fun':dispatch_message coroutine_resume]--['self:']",skynet.self())
		-- else
		-- end
