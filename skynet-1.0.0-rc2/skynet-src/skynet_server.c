...
skynet_context_push(uint32_t handle, struct skynet_message *message) {
	int type = message->sz >> MESSAGE_TYPE_SHIFT;
	struct skynet_context * ctx = skynet_handle_grab(handle);

	// if(type == 6 && handle == 13 && message->source == 0)
	// {
	// 	printf("[:'log']--['file':%s]--['fun':%s]--['type':%d]--['MESSAGE_TYPE_SHIFT':%lu]\n","skynet_server.c","skynet_context_push",type,MESSAGE_TYPE_SHIFT);
	// 	printf("[:'log']--['file':%s]--['fun':%s]--['source':%d]--['handle':]%d\n","skynet_server.c","skynet_context_push",message->source,handle);
	// }
...
dispatch_message(struct skynet_context *ctx, struct skynet_message *msg) {
	...
	// printf("[:'log']--['file':%s]--['fun':%s]--['type':%d]--['MESSAGE_TYPE_SHIFT':%lu]\n","skynet_server.c","dispatch_message",type,MESSAGE_TYPE_SHIFT);
...
cmd_reg(struct skynet_context * context, const char * param) {
	if (param == NULL || param[0] == '\0') {
		...
		// printf("[:'log']--['file':%s]--['fun':%s]--['context->result:'%s]\n","skynet_server.c","cmd_reg",context->result);
		return context->result;
	} else if (param[0] == '.') {
...
skynet_send(struct skynet_context * context, uint32_t source, uint32_t destination , int type, int session, void * data, size_t sz) {
	// if(type == 6)
	// {
	// 	printf("[:'log']--['file':%s]--['fun':%s]--['context->handle':%d]\n","skynet_server.c","skynet_send",context->handle);
	// 	printf("[:'log']--['file':%s]--['fun':%s]--['source':%d]--['destination':%d]\n","skynet_server.c","skynet_send",source,destination);
	// }
...
skynet_context_send(struct skynet_context * ctx, void * msg, size_t sz, uint32_t source, int type, int session) {
	printf("[:'log']--['file':%s]--['fun':%s]--['type':%d]--['MESSAGE_TYPE_SHIFT':%lu]\n","skynet_server.c","skynet_context_send",type,MESSAGE_TYPE_SHIFT);
...
skynet_initthread(int m) {
	// printf("[:log]--[file:%s]--[fun:%s]--[m:%d]\n","skynet_server.c","skynet_initthread",m);
	uintptr_t v = (uint32_t)(-m);
	// printf("[:log]--[file:%s]--[fun:%s]--[v:%u]\n","skynet_server.c","skynet_initthread",v);
